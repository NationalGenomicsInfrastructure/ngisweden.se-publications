...
async function fetchPublicationsFromAPI(facility: string, downloadLimit: number): Promise<Publication[]> {
...
function processPublications(publications: Publication[], options: PublicationOptions): Publication[] {
    const processedPubs = publications.map(pub => {
        // Check for collaborations
        const isCollab = FACILITY_LABELS.some(fac => 
            pub.labels[fac] === 'Collaborative'
        );
        
        // Check for technology development
        const isTechDev = FACILITY_LABELS.some(fac => 
            pub.labels[fac] === 'Technology development'
        );
    ...
    // Remove duplicates
    const uniquePubs = processedPubs.filter((pub, index, self) =>
        index === self.findIndex(p => 
            p.iuid === pub.iuid || p.doi === pub.doi
        )
    );
...
function formatPublicationHTML(publication: Publication): string {
    const authors = publication.authors.map((author: Author) => {
        const given = author.given === author.given.toUpperCase() 
            ? author.given.toLowerCase().replace(/\b\w/g, (l: string) => l.toUpperCase())
            : author.given;
        const family = author.family === author.family.toUpperCase()
            ? author.family.toLowerCase().replace(/\b\w/g, (l: string) => l.toUpperCase())
            : author.family;
...
export async function getPublications(
    ...
    try {
        ...
        const listItems = publications.map(pub => `
